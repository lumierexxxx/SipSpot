...
reviewSchema.virtual('helpfulPercentage').get(function(this: any) {
...
// Get sentiment emoji
reviewSchema.virtual('sentimentEmoji').get(function(this: any) {
...
reviewSchema.virtual('ratingDisplay').get(function(this: any) {
...
// Time since posted
reviewSchema.virtual('timeAgo').get(function(this: any) {
    const now = new Date();
    const diff = now.getTime() - (this.createdAt as Date).getTime();
 ...
 */
reviewSchema.methods.addAIAnalysis = async function(analysisData: Record<string, unknown>) {
    this.aiAnalysis = {
 ...
 */
reviewSchema.methods.addHelpfulVote = async function(userId: unknown, voteType: string) {
    // Check if user already voted
    const existingVoteIndex = this.helpfulVotes.findIndex(
        (vote: { user: { toString(): string } }) => vote.user.toString() === (userId as { toString(): string }).toString()
    );
 ...
 * Remove helpful vote
 */
reviewSchema.methods.removeHelpfulVote = async function(userId: unknown) {
    const existingVoteIndex = this.helpfulVotes.findIndex(
        (vote: { user: { toString(): string } }) => vote.user.toString() === (userId as { toString(): string }).toString()
    );
 ...
 * Add owner response
 */
reviewSchema.methods.addOwnerResponse = async function(content: string, responderId: unknown) {
    this.ownerResponse = {
        content,
 ...
 */
reviewSchema.statics.getByCafe = function(cafeId: unknown, options: { page?: number; limit?: number; sort?: string } = {}) {
    const {
        page = 1,
        limit = 10,
        sort = '-createdAt'
 ...
 * Get user's reviews
 */
reviewSchema.statics.getByUser = function(userId: unknown, options: { page?: number; limit?: number } = {}) {
 ...
 */
reviewSchema.statics.getMostHelpful = function(cafeId: unknown, limit = 5) {
    return this.find({ cafe: cafeId })
        .populate('author', 'username avatar')
 ...
 */
reviewSchema.statics.calculateAverageRating = async function(cafeId: unknown) {
 ...
 */
reviewSchema.statics.getSentimentStats = async function(cafeId: unknown) {
    const stats = await this.aggregate([
...
reviewSchema.post('save', async function(doc) {
    try {
        ...
        if (cafe) {
            await (cafe as unknown as { calculateAverageRating(): Promise<void> }).calculateAverageRating();
        }
    } catch (error) {
