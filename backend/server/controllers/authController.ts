...
export const register = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (existingUser) {
            const field = (existingUser as any).email === email.toLowerCase() ? '邮箱' : '用户名';
...
export const login = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // 查找用户（包含密码字段）
        const user = await (User as any).findByEmailOrUsername(identifier);
...
export const updateProfile = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (updates.username || updates.email) {
            ...
            if (existingUser) {
                const field = (existingUser as any).username === updates.username ? '用户名' : '邮箱';
                return next(new ExpressError(`${field}已被使用`, 400));
            }
        }
...
export const resetPassword = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // 查找用户
        const user = await (User as any).findByResetToken(token) as any;
